...
class SessionFlowsTest < ActionDispatch::IntegrationTest
  ...
  test 'should not get new room page without logging in' do
    get new_room_url(:en)
    assert_redirected_to new_user_session_url(:en)
  end
  ...
  test 'should not get edit profile page without logging in' do
    get edit_user_registration_url(:en)
    assert_redirected_to new_user_session_url(:en)
  end
  ...
  test 'user with incorrect credentials will not be redirected' do
    post user_session_url(:en), params: { email: Faker::Internet.email, password: Faker::Lorem.word }
    assert_response :success
  end
  ...
  test 'user can create a room' do
    sign_in users(:valid)
    post rooms_url(:en), params: { room: { name: 'test' }, table: { width: 2 } }
  ...
  test 'user can logout' do
    ...
    delete destroy_user_session_url(:en)
    assert_redirected_to root_url(:en)
  end
end
